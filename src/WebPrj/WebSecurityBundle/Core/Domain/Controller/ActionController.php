...
use WebPrj\WebSecurityBundle\Entity\Action;
...
class ActionController extends Controller
{
     ...
     *
     * @Route("/", name="ums_sec_actions_index")
     * @Method("GET")
     */
    public function indexAction()
    {
        ...
        $actions = $em->getRepository('WebPrjWebSecurityBundle:Action')->findAll();
     ...
     *
     * @Route("/new", name="ums_sec_actions_new")
     * @Method({"GET", "POST"})
     */
    public function newAction(Request $request)
    {
        $action = new Action();
        $form = $this->createForm('WebPrj\WebSecurityBundle\Form\ActionType', $action);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('ums_sec_actions_show', array('id' => $action->getId()));
        }
     ...
     *
     * @Route("/{id}", name="ums_sec_actions_show")
     * @Method("GET")
     */
     ...
     *
     * @Route("/{id}/edit", name="ums_sec_actions_edit")
     * @Method({"GET", "POST"})
     */
    public function editAction(Request $request, Action $action)
    {
        $deleteForm = $this->createDeleteForm($action);
        $editForm = $this->createForm('WebPrj\WebSecurityBundle\Form\ActionType', $action);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('ums_sec_actions_edit', array('id' => $action->getId()));
        }
     ...
     *
     * @Route("/{id}", name="ums_sec_actions_delete")
     * @Method("DELETE")
     */
    public function deleteAction(Request $request, Action $action)
    {
        ...
        return $this->redirectToRoute('ums_sec_actions_index');
    }
    ...
    private function createDeleteForm(Action $action)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('ums_sec_actions_delete', array('id' => $action->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
